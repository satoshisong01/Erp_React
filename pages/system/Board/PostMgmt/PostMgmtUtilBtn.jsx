...
import PostMgmtModalPagePost from "./PostMgmtModalPagePost";

export default function PostMgmtUtilBtn({
    initialData,
    refresh,
    changeInt,
    selectedData,
    ...
    const [PostMgmt, setPostMgmt] = useState(1); //키 값 넘버자동 1씩추가
    ...
    //const changeInt = selectedData.map((item) => item.PostMgmt); //체크된 데이터 배열로 담아서 저장
    ...
    const body = selectedData.map((item) =>
        [
            { v: item.PostMgmt, t: "s" },
            { v: item.PostMgmtNm, t: "s" },
            { v: item.PostMgmtDc, t: "s" },
    ...
    const handleDelete = async () => {
        try {
            ...
            const response = await axios.delete(
                `http://192.168.0.113:8080/api/system/code/PostMgmt/removeAll.do`,
                //`http://localhost:8080/api/system/code/PostMgmt/removeAll.do`,
                options
            );
    ...
    return (
        <div>
            ...
            {postModalOpen && (
                <PostMgmtModalPagePost
                    onClose={() => {
                        setPostModalOpen(false);
                    }}
                    refresh={refresh}
                    countPostMgmt={PostMgmt}
                    urlName={urlName}
                    headers={headers}
                />
            )}
