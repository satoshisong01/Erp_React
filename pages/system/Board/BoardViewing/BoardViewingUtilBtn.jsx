...
import BoardViewingModalPagePost from "./BoardViewingModalPagePost";

export default function BoardViewingUtilBtn({
    initialData,
    refresh,
    changeInt,
    selectedData,
    ...
    //const changeInt = selectedData.map((item) => item.BoardViewing); //체크된 데이터 배열로 담아서 저장
    ...
    const body = selectedData.map((item) =>
        [
            { v: item.BoardViewing, t: "s" },
            { v: item.BoardViewingNm, t: "s" },
            { v: item.BoardViewingDc, t: "s" },
    ...
    const handleDelete = async () => {
        try {
            ...
            const response = await axios.delete(
                `http://192.168.0.113:8080/api/system/code/BoardViewing/removeAll.do`,
                //`http://localhost:8080/api/system/code/BoardViewing/removeAll.do`,
                options
            );
    ...
    const handleModalPostClick = (e) => {
        ...
        //console.log(item);
        //setModalItem(item);
        setBoardViewing(BoardViewing + 1);
    ...
    return (
        <div>
            ...
            {postModalOpen && (
                <BoardViewingModalPagePost
                    onClose={() => {
                        setPostModalOpen(false);
                    }}
                    refresh={refresh}
                    countBoardViewing={BoardViewing}
                    urlName={urlName}
                    headers={headers}
                />
            )}
