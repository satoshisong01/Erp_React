...
import BoardViewingModalPage from "./BoardViewingModalPage";
import BoardViewingUtilBtn from "./BoardViewingUtilBtn";
import BoardViewingTableSearchBar from "./BoardViewingTableSearchBar";
...
const BoardViewings = () => {
    ...
    //체크된 아이템의 BoardViewing 숫자만 저장
    const changeInt = selectedData.map((item) => item.BoardViewing);
    ...
    const handleItemCheck = (item, e) => {
        ...
        setSelectedData((prevSelectedData) => {
            if (isChecked) {
                ...
                if (!prevSelectedData.find((selectedItem) => selectedItem.BoardViewing === item.BoardViewing)) {
                    const sortedData = [...prevSelectedData, item].sort((a, b) => {
                        // BoardViewing 속성을 기준으로 데이터 정렬
                        if (a.BoardViewing < b.BoardViewing) {
                            return -1;
                        }
                        if (a.BoardViewing > b.BoardViewing) {
                            return 1;
                ...
                return prevSelectedData.filter((selectedItem) => selectedItem.BoardViewing !== item.BoardViewing);
            }
                                            ...
                                            <table ref={dataTableRef} className="table table-bordered" id="dataTable">
                                                <thead>
                                                    <tr>
                                                        ...
                                                        {["게시판명", "권한명", "그룹권한명", "댓글", "수정일", "수정자", "사용여부"].map((item, index) => (
                                                            ...
                                                            <td>
                                                                <input
                                                                    ...
                                                                    checked={selectedData.some(
                                                                        (selectedItem) => selectedItem.BoardViewing === item.BoardViewing
                                                                    )}
