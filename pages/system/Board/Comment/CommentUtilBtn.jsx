...
import CommentModalPagePost from "./CommentModalPagePost";

export default function CommentUtilBtn({
    initialData,
    refresh,
    changeInt,
    selectedData,
    ...
    const [Comment, setComment] = useState(1); //키 값 넘버자동 1씩추가
    ...
    //const changeInt = selectedData.map((item) => item.Comment); //체크된 데이터 배열로 담아서 저장
    ...
    const body = selectedData.map((item) =>
        [
            { v: item.Comment, t: "s" },
            { v: item.CommentNm, t: "s" },
            { v: item.CommentDc, t: "s" },
    ...
    const handleDelete = async () => {
        try {
            ...
            const response = await axios.delete(
                `http://192.168.0.113:8080/api/system/code/Comment/removeAll.do`,
                //`http://localhost:8080/api/system/code/Comment/removeAll.do`,
                options
            );
