...
import AccessHistoryModalPage from "./AccessHistoryModalPage";
import AccessHistoryUtilBtn from "./AccessHistoryUtilBtn";
import AccessHistoryTableSearchBar from "./AccessHistoryTableSearchBar";

const AccessHistorys = () => {
    ...
    //체크된 아이템의 AccessHistory 숫자만 저장
    const changeInt = selectedData.map((item) => item.AccessHistory);
    ...
    const handleItemCheck = (item, e) => {
        ...
        setSelectedData((prevSelectedData) => {
            if (isChecked) {
                ...
                if (!prevSelectedData.find((selectedItem) => selectedItem.AccessHistory === item.AccessHistory)) {
                    const sortedData = [...prevSelectedData, item].sort((a, b) => {
                        // AccessHistory 속성을 기준으로 데이터 정렬
                        if (a.AccessHistory < b.AccessHistory) {
                            return -1;
                        }
                        if (a.AccessHistory > b.AccessHistory) {
                            return 1;
                ...
                return prevSelectedData.filter((selectedItem) => selectedItem.AccessHistory !== item.AccessHistory);
            }
                                            ...
                                            <table ref={dataTableRef} className="table table-bordered" id="dataTable">
                                                <thead>
                                                    <tr>
                                                        ...
                                                        {["접속 ID", "접속이름", "로그시간", "로그정보", "접속 IP", "최근접속일시", "결과"].map(
                                                            (item, index) => (
                                                            ...
                                                            <td>
                                                                <input
                                                                    ...
                                                                    checked={selectedData.some(
                                                                        (selectedItem) => selectedItem.AccessHistory === item.AccessHistory
                                                                    )}
