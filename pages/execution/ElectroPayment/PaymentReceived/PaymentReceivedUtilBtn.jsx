...
import PaymentReceivedModalPagePost from "./PaymentReceivedModalPagePost";

export default function PaymentReceivedUtilBtn({
    initialData,
    refresh,
    changeInt,
    selectedData,
    ...
    const [PaymentReceived, setPaymentReceived] = useState(1); //키 값 넘버자동 1씩추가
    const [postModalOpen, setPostModalOpen] = useState(false); // 클릭 추가 모달창

    //버튼 활성화 (코드관리에서는 엑셀버튼은 비활성화)
    const showExcelBtn =
        urlName === "PaymentReceiveds" ||
        urlName === "groupCode" ||
        urlName === "detailCode"
            ? true
            : false;
    ...
    const handleModalPostClick = (e) => {
        ...
        //console.log(item);
        //setModalItem(item);
        setPaymentReceived(PaymentReceived + 1);
    ...
    return (
        <div>
            ...
            {postModalOpen && (
                <PaymentReceivedModalPagePost
                    onClose={() => {
                        setPostModalOpen(false);
                    }}
                    refresh={refresh}
                    countPaymentReceived={PaymentReceived}
                    urlName={urlName}
                    headers={headers}
